...
def convert_to_xml(result_path: str, gold_path: str, max_length_diff=None):
    output_dir = result_path.split("/")[-2]
    _save_path = f"../temp_output/xml/{output_dir}"
    os.makedirs(_save_path, exist_ok=True)
    pred_xml_path = os.path.join(_save_path, f"pred.xml")
    ref_xml_path = os.path.join(_save_path, f"ref.xml")

    pred_triplets = [l.strip() for l in open(result_path, "r", encoding='utf-8').readlines()]
    gold_triplets = [l.strip() for l in open(gold_path, "r", encoding='utf-8').readlines()]
...
def calculateAllScores(newreflist, newcandlist):
    ...
    # for idx, candidate in enumerate(tqdm(newcandlist, desc="Processing calculateAllScores")):
    #     candidatesemeval = []
    ...
    def process_one(idx, candidate):
        ...
        for triple in candidate:
            ...
            candidatesemeval.append(triplesemeval)
            candidatesemevalpertag.append(triplesemevalpertag)
        return candidatesemeval, candidatesemevalpertag

    with ThreadPoolExecutor() as executor:
        futures = [executor.submit(process_one, idx, candidate) for idx, candidate in enumerate(newcandlist)]
        for future in tqdm(futures, desc="Processing calculateAllScores"):
            candidatesemeval, candidatesemevalpertag = future.result()
            totalsemevallist.append(candidatesemeval)
            totalsemevallistpertag.append(candidatesemevalpertag)
...
def calculateSystemScore(totalsemevallist, totalsemevallistpertag, newreflist, newcandlist):
    ...
    my_eval_scores = []
    for idx, candidate in enumerate(newcandlist):
        cands = len(newcandlist[idx])
        refs = len(newreflist[idx])
        size = max(cands, refs)
        ...
        for i in range(cands):
            for j in range(refs):
                ...
                f1score = statistics.mean(
                    [scores["ent_type"]["f1"], scores["partial"]["f1"], scores["strict"]["f1"], scores["exact"]["f1"]]
                )
                cost_matrix[i, j] = -f1score
        ...
        totalscore = 0
        partial_f1, strict_f1, exact_f1 = 0,0,0

        for r, c in zip(row_ind, col_ind):
            if r < cands and c < refs:
                scores = totalsemevallist[idx][r][c]
                f1score = statistics.mean(
                    [scores["ent_type"]["f1"], scores["partial"]["f1"], scores["strict"]["f1"], scores["exact"]["f1"]]
                )
                totalscore += f1score

                partial_f1 +=scores["partial"]["f1"]
                strict_f1 +=scores["strict"]["f1"]
                exact_f1 +=scores["exact"]["f1"]

                collectedsemeval.append(scores)
                collectedsemevalpertag.append(totalsemevallistpertag[idx][r][c])

        selectedsemevallist += collectedsemeval
        selectedsemevallistpertag += collectedsemevalpertag
        selectedalignment.append((row_ind.tolist(), col_ind.tolist()))
        selectedscores.append(totalscore / len(candidate))
        my_eval_scores.append((partial_f1 / len(candidate),strict_f1 / len(candidate),exact_f1 / len(candidate)))
    ...
    #     results = []
    #     with ThreadPoolExecutor() as executor:
    #         futures = [executor.submit(process_combination, combination) for combination in combilist]
    #         for future in tqdm(futures, desc="Processing combinations"):
    #             results.append(future.result())
    #
    #     totaldict = {}
    #     for res in results:
    #         if ("totalscore" not in totaldict) or (res["totalscore"] > totaldict["totalscore"]):
    #             totaldict = res
    ...
    def print_sys_evel(selectedsemevallist, selectedsemevallistpertag):
        print("-----------------------------------------------------------------")
        print("Total scores")
        print("-----------------------------------------------------------------")
        print("Ent_type")
        ...
        print("-----------------------------------------------------------------")
        print("\n-----------------------------------------------------------------")
        print("Markdown Table Format Evaluation Results")
        print("-----------------------------------------------------------------")

        # Function to generate a markdown table row for a given metric set
        def generate_table_row(name, correct, incorrect, partial, missed, spurious, possible, actual, precision, recall, f1):
            return (
                f"| {name} "
                f"| {correct} | {incorrect} | {partial} | {missed} "
                f"| {spurious} | {possible} | {actual} "
                f"| {precision:.3f} | {recall:.3f} | {f1:.3f} |"
            )

        # Total Scores Table
        print("\n### Total Scores")
        print("| Metric Type | Correct | Incorrect | Partial | Missed | Spurious | Possible | Actual | Precision | Recall | F1 |")
        print("|-------------|---------|-----------|---------|--------|----------|----------|--------|-----------|--------|----|")
        print(generate_table_row(
            "Ent_type", enttypecorrect, enttypeincorrect, enttypepartial, enttypemissed,
            enttypespurious, enttypepossible, enttypeactual, enttypeprecision, enttyperecall, enttypef1
        ))
        print(generate_table_row(
            "Partial", partialcorrect, partialincorrect, partialpartial, partialmissed,
            partialspurious, partialpossible, partialactual, partialprecision, partialrecall, partialf1
        ))
        print(generate_table_row(
            "Strict", strictcorrect, strictincorrect, strictpartial, strictmissed,
            strictspurious, strictpossible, strictactual, strictprecision, strictrecall, strictf1
        ))
        print(generate_table_row(
            "Exact", exactcorrect, exactincorrect, exactpartial, exactmissed,
            exactspurious, exactpossible, exactactual, exactprecision, exactrecall, exactf1
        ))

        # Scores per tag - Subjects Table
        print("\n### Scores per tag - Subjects")
        print("| Metric Type | Correct | Incorrect | Partial | Missed | Spurious | Possible | Actual | Precision | Recall | F1 |")
        print("|-------------|---------|-----------|---------|--------|----------|----------|--------|-----------|--------|----|")
        print(generate_table_row(
            "Ent_type", subenttypecorrect, subenttypeincorrect, subenttypepartial, subenttypemissed,
            subenttypespurious, subenttypepossible, subenttypeactual, subenttypeprecision, subenttyperecall, subenttypef1
        ))
        print(generate_table_row(
            "Partial", subpartialcorrect, subpartialincorrect, subpartialpartial, subpartialmissed,
            subpartialspurious, subpartialpossible, subpartialactual, subpartialprecision, subpartialrecall, subpartialf1
        ))
        print(generate_table_row(
            "Strict", substrictcorrect, substrictincorrect, substrictpartial, substrictmissed,
            substrictspurious, substrictpossible, substrictactual, substrictprecision, substrictrecall, substrictf1
        ))
        print(generate_table_row(
            "Exact", subexactcorrect, subexactincorrect, subexactpartial, subexactmissed,
            subexactspurious, subexactpossible, subexactactual, subexactprecision, subexactrecall, subexactf1
        ))

        # Scores per tag - Predicates Table
        print("\n### Scores per tag - Predicates")
        print("| Metric Type | Correct | Incorrect | Partial | Missed | Spurious | Possible | Actual | Precision | Recall | F1 |")
        print("|-------------|---------|-----------|---------|--------|----------|----------|--------|-----------|--------|----|")
        print(generate_table_row(
            "Ent_type", predenttypecorrect, predenttypeincorrect, predenttypepartial, predenttypemissed,
            predenttypespurious, predenttypepossible, predenttypeactual, predenttypeprecision, predenttyperecall, predenttypef1
        ))
        print(generate_table_row(
            "Partial", predpartialcorrect, predpartialincorrect, predpartialpartial, predpartialmissed,
            predpartialspurious, predpartialpossible, predpartialactual, predpartialprecision, predpartialrecall, predpartialf1
        ))
        print(generate_table_row(
            "Strict", predstrictcorrect, predstrictincorrect, predstrictpartial, predstrictmissed,
            predstrictspurious, predstrictpossible, predstrictactual, predstrictprecision, predstrictrecall, predstrictf1
        ))
        print(generate_table_row(
            "Exact", predexactcorrect, predexactincorrect, predexactpartial, predexactmissed,
            predexactspurious, predexactpossible, predexactactual, predexactprecision, predexactrecall, predexactf1
        ))

        # Scores per tag - Objects Table
        print("\n### Scores per tag - Objects")
        print("| Metric Type | Correct | Incorrect | Partial | Missed | Spurious | Possible | Actual | Precision | Recall | F1 |")
        print("|-------------|---------|-----------|---------|--------|----------|----------|--------|-----------|--------|----|")
        print(generate_table_row(
            "Ent_type", objenttypecorrect, objenttypeincorrect, objenttypepartial, objenttypemissed,
            objenttypespurious, objenttypepossible, objenttypeactual, objenttypeprecision, objenttyperecall, objenttypef1
        ))
        print(generate_table_row(
            "Partial", objpartialcorrect, objpartialincorrect, objpartialpartial, objpartialmissed,
            objpartialspurious, objpartialpossible, objpartialactual, objpartialprecision, objpartialrecall, objpartialf1
        ))
        print(generate_table_row(
            "Strict", objstrictcorrect, objstrictincorrect, objstrictpartial, objstrictmissed,
            objstrictspurious, objstrictpossible, objstrictactual, objstrictprecision, objstrictrecall, objstrictf1
        ))
        print(generate_table_row(
            "Exact", objexactcorrect, objexactincorrect, objexactpartial, objexactmissed,
            objexactspurious, objexactpossible, objexactactual, objexactprecision, objexactrecall, objexactf1
        ))
    print_sys_evel(selectedsemevallist, selectedsemevallistpertag)
    # print("-" * 100, "\n", f"\033[31m{my_eval_scores}\033[0m")
...
if __name__ == "__main__":
    ...
    choose_dataset = _datasets[1]

    parser.add_argument("--edc_output", default=f"")


    parser.add_argument("--reference", default=f"./references/{choose_dataset}.txt")
